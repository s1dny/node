...
Personal homelab configuration for a Dell Optiplex 7060 (`azalab-0`) running NixOS with k3s. This repo is a Nix flake that exports a NixOS module (`nixosModules.default`) consumed by a host bootstrap flake on the server at `/etc/nixos/flake.nix`. The repo is never cloned on the host: NixOS pins it via `flake.lock` and mounts the pinned source read-only at `/etc/homelab/source`.
