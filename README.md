...
- `/etc/homelab/source/flux/clusters/<cluster>/manifests/apps/<app>/`: app manifests and per-app k8s secret templates
...
6. From your local machine (in the root of your clone of this repo), create host/service secrets and per-secret k8s env files:
   ...
   mkdir -p flux/live
   for f in flux/clusters/${CLUSTER}/manifests/apps/*/*.env.example; do cp "$f" "flux/live/$(basename "${f%.example}")"; done
   vi flux/live/*.env
